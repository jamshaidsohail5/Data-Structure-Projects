...
public:
	...
	//copy constructor
	n_dim_dynamic_array(const n_dim_dynamic_array& rightobj)
	{
		size_of_array_of_dimentions=rightobj.size_of_array_of_dimentions;
		size_of_data_of_dimentions=rightobj.size_of_data_of_dimentions;

		array_of_dimentions=new int[size_of_array_of_dimentions]();
		data_of_dimentions= new type[size_data_of_dimentions]();

		for(int i=0;i<size_of_array_of_dimentions;i++)
		{
			array_of_dimentions[i]=rightobj.array_of_dimentions[i];
		}
		for(int i=0;i<size_of_data_of_dimentions;i++)
		{
			data_of_dimentions[i]=rightobj.data_of_dimentions[i];
		}

	}
	//operator =
	const n_dim_dynamic_array<type> & operator=(const n_dim_dynamic_array<type> & rightobj)
	{
		if(this !=& rightobj)
		{
			delete [] array_of_dimentions;
